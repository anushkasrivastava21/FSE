...
async function main() {
  ...
  // ------------------------------------------------------------
  // 6. Deploy ForecastRegistry (Person C — standalone contract)
  // ------------------------------------------------------------
  console.log("\n6. Deploying ForecastRegistry...");

  const ForecastRegistry =
    await ethers.getContractFactory("ForecastRegistry");

  const forecastRegistry = await ForecastRegistry.deploy();

  await forecastRegistry.waitForDeployment();

  const forecastRegistryAddress =
    await forecastRegistry.getAddress();

  console.log("ForecastRegistry:", forecastRegistryAddress);

  // ------------------------------------------------------------
  // 7. Wire Listing
  //    MatchingEngine + Settlement may update listing status
  // ------------------------------------------------------------
  console.log("\n7. Authorizing MatchingEngine + Settlement in Listing...");
  ...
  // ------------------------------------------------------------
  console.log("\n8. Authorizing MatchingEngine in Order...");
  ...
  // ------------------------------------------------------------
  // 9. Wire FoodCreditToken
  //    Settlement may mint Food Credit NFTs
  // ------------------------------------------------------------
  console.log("\n9. Authorizing Settlement in FoodCreditToken...");
  ...
  deployments[key] = {
    ...
    ForecastRegistry: forecastRegistryAddress,
